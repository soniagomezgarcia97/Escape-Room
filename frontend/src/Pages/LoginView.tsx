  ...
  return (
    <>
      <div className="w-full h-screen flex flex-col justify-center items-center gap-10">
        <form onSubmit={handleSubmit(handleLogin)} className="w-full px-5 rounded-lg space-y-10">
          ...
          <button type="submit" className="bg-yellow-500 hover:bg-yellow-600 text-[#140C08] font-bold py-2 px-4 rounded">Iniciar sesión</button>
        </form>
        <nav className="w-full flex flex-row justify-center gap-2">
          <span className="flex flex-row justify-center text-slate-500">¿No tienes una cuenta?</span><Link to="/auth/register" className="text-yellow-500">Crea una cuenta</Link>
        </nav>
      </div>
    </>
  )
